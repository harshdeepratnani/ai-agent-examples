...
def book_cab(origin: str, destination: str):
    """ Tool to book a cab for users from source to destination """
    response = f"Successfully booked a cab from {origin} to {destination}"
    return response
...
llm_with_tools = llm.bind_tools([book_cab])
...
def human_input(state: MessagesState):
    ...
    result = interrupt({
        "question": "Do you want me to proceed with booking your cab?",
        "actions": ["Yes", "No"]
    })
    if result == 'abort':
        goto = END
...
builder.add_node("tools", ToolNode([book_cab]))
...
graph = builder.compile(checkpointer=memory, name="cab_booking_agent")

# Draw the graph
try:
    graph.get_graph(xray=True).draw_mermaid_png(output_file_path="cab_booking_agent.png")
except Exception:
    pass
